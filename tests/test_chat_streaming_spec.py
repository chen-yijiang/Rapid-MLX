...
def test_terminal_replay_guard_handles_split_special_token(monkeypatch):
    """#1709 (codex): a removable special token split across two content
    deltas defeats a per-delta sanitize (neither half strips alone), so the
    accumulated wire text still carries it while the buffer sanitized as one
    string drops it. The guard must re-sanitize the accumulated wire text as a
    whole so both operands collapse to the same form; otherwise the terminal
    snapshot is replayed. Streamed ``"hi<|im_"`` + ``"end|>"`` (buffer
    ``"hi<|im_end|>"``) + finalize flush of the buffer → no replay.
    """
    from vllm_mlx.domain.events import StreamEvent
    from vllm_mlx.service import postprocessor as pp_mod
    ...
    def finalize_flush_buffer(self):
        self.tool_accumulated_text = buf
        return [StreamEvent(type="content", content=buf)]

    monkeypatch.setattr(
        pp_mod.StreamingPostProcessor, "finalize", finalize_flush_buffer
    )

    class _SplitTokenEngine(_PlainStreamEngine):
        async def stream_chat(self, messages, **kwargs):
            accumulated = ""
            for i, delta in enumerate(["hi<|im_", "end|>"]):
                accumulated += delta
                yield GenerationOutput(
                    text=accumulated,
                    new_text=delta,
    ...
    # The terminal flush of the whole buffer must be recognised as a replay
    # even though the token was split across deltas: no second copy appended.
    assert joined.count("hi") == 1, (
        f"a split-special-token snapshot must still be suppressed; got {joined!r}"
    )
