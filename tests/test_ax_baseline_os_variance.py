...
def test_expanded_system_scrollbar_is_dropped_but_leaf_bar_remains():
    ...
    labelled = ax_baseline.Node(
        {
            "role": "AXScrollBar",
            "identifier": "Transcript.Position",
            "description": "Transcript position",
        }
    )
    root.children = [labelled]
    assert ax_baseline.render(root, ()) == [
        "AXScrollArea",
        '  AXScrollBar id="Transcript.Position" desc="Transcript position"',
    ]


def test_search_overlay_order_is_canonical_but_other_siblings_keep_order():
    before = ax_baseline.Node({"role": "AXButton", "identifier": "Before"})
    panel = ax_baseline.Node(
        {"role": "AXUnknown", "identifier": "ConversationSearch.Panel"}
    )
    results = ax_baseline.Node({"role": "AXScrollArea"})
    main = ax_baseline.Node(
        {"role": "AXSplitGroup", "identifier": "main, SidebarNavigationSplitView"}
    )
    after = ax_baseline.Node({"role": "AXButton", "identifier": "After"})

    macos15 = ax_baseline.normalize_hosting_children(
        [before, main, panel, results, after]
    )
    macos26 = ax_baseline.normalize_hosting_children(
        [before, panel, results, main, after]
    )

    assert macos15 == macos26 == [before, panel, results, main, after]
